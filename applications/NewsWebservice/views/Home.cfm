...
<cfoutput>	
	...
	<p>When you click on "Get Feed (Using Clientstorage)" the news will be saved using the client storage.  You will need to have a clientstorage setup already in order to use this.  The news now become permanent until you clean them.</p>
	</td>
	</tr>
	<tr>
	<td>
		<form id="form1" name="form1" method="post" action="" onSubmit="disableButtons()">
		  ...
		  <input type="submit" name="getclientbutton" value="Get News Feed (Using ClientStorage)" onclick="changeEvent('#Event.getValue("xehGetNewsClient")#')" />
		  <input name="event" type="hidden" id="event" value="#Event.getValue("xehGetNews")#" />
		</form>
		<p >&nbsp;</p>
		<!--- Render a MessageBox if set --->
		#getInstance( "messagebox@cbmessagebox" ).renderit()#<br>
	</td>
	</tr>
	</table>
	...
	<!--- Display the news if found in the client storage --->
	<cfif getInstance( "ClientStorage@cbstorages" ).exists("newsfeed")>
		<div class="newsbox">
		...
		<cfdump var="#getInstance( "ClientStorage@cbstorages" ).getvar( "newsfeed" )#">
		</div>
	</cfif>
