...
<!------------------------------------------- PRIVATE EVENTS ------------------------------------------>
    ...
    <cfreturn getPlugin(plugin="Utilities",customPlugin=true).numberSuffix(arguments.number)/>
  </cffunction>

	<cffunction name="numSep" access="private" returntype="string" output="false">
  	<cfargument name="number" type="numeric" required="yes">
    <cfreturn getPlugin(plugin="Utilities",customPlugin=true).number1kSeparator(arguments.number)/>
  </cffunction>	
