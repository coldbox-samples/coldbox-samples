...
<cffunction name="init" output="false" returntype="any" hint="I initialize the config bean.">
	<cfscript>
		...
		var cfcModelRoot = getController().getSetting('cfcroot');

		// BEAN DEFINITIONS (see top of bean for instructions)
		// Product Service
		addSingleton("#cfcModelRoot#.com.model.product.ProductService");
		...
		// Product DAO
		addSingleton("#cfcModelRoot#.com.model.product.ProductDAO","ProdDAO");

		// Product
		addTransient("#cfcModelRoot#.com.model.product.ProductBean","Product");
		addConstructorDependency("Product","ProdDAO");

		// Category Service
		addSingleton("#cfcModelRoot#.com.model.category.CategoryService");
		...
		addSingleton("#cfcModelRoot#.com.model.category.CategoryDAO");
